...
The key design feature of Kontra's `Checkbox` is that it **directly manipulates** an existing boolean variable in your application, creating a seamless two-way data binding.
...
Like the `Button`, this builder composes styles created with the `StyleBuilder`.
...
<CodeBlock border 
  ...
  code={`
    ...
    int main() {
        ...
        auto layout = std::make_shared<List>({display, chk1, chk2, chk3});
...
- A simple checkbox to toggle a state!
