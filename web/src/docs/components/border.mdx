...
<CodeBlock
  lang="cpp"
  code={`auto border = std::make_shared<Border>(child, border_style);`}
/>
...
<CodeBlock
  lang="cpp"
  code={`auto border_style = BorderStyleBuilder()
...
<CodeBlock
  lang="cpp"
  code={`auto message = std::make_shared<Text>("Operation completed successfully!");
